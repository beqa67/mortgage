  ...
  <div class="form-block" :class="{ 'form-block--mobile': mobileVersion }">
    ...
    <div class="form-block__footer">
      ...
      <div class="form-block__footer-info-item d-flex justify-content-between">
        ...
        >{{loadCurrency[choosenCurrency].symbol}} {{montlyPayment}}</div>
      </div>
...
export default {
  ...
  methods: {
    ...
    changeChoosenCurrencyId(e) {
      ...
      let currentCurrencyState = JSON.parse(JSON.stringify(this.loadCurrency));

      for (let i = 0; i < currentCurrencyState.length; i++) {
        currentCurrencyState[i].checked = false;
      }
      currentCurrencyState[choosenCurrencyOptionValue].checked = true;
      let updatedCurrencystate = currentCurrencyState;

      this.$store.dispatch("setChoosenCurrency", updatedCurrencystate);
      this.checkChoosenCurrency();
    },
    checkChoosenCurrency() {
      for (let i = 0; i < this.loadCurrency.length; i++) {
        if (this.loadCurrency[i].checked == true) {
          this.choosenCurrency = this.loadCurrency[i].id;
        }
      }
      this.$store.dispatch("setChoosenCurrencyId", this.choosenCurrency);
      this.setInterestRate();
    },
    setInterestRate() {
      let currentBanksInterests = [];
      let banksLength = this.choosenBanksId.length;
      let indexCounter = 0;
      for (let bank of this.loadData.banks) {
        for (let choosenBankItem of this.choosenBanksId) {
          if (choosenBankItem == bank.id) {
            if (this.choosenCurrencyId == 0) {
              currentBanksInterests.push(parseFloat(bank[1]));
            } else if (this.choosenCurrencyId == 1) {
              currentBanksInterests.push(parseFloat(bank[2]));
            } else if (this.choosenCurrencyId == 2) {
              currentBanksInterests.push(parseFloat(bank.percent_eur));
            }
          }
        }
        indexCounter++;
      }
      let minInterest = Math.min.apply(Math, currentBanksInterests);
      this.$store.dispatch("setInterestRate", minInterest);
      this.calculateMontlyPayment();
    },
